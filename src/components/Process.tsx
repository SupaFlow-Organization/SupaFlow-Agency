...
function ProcessCard({ step, index }: { step: typeof steps[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="process-card h-[24rem] sm:h-[28rem] bg-surface border border-black/5 rounded-2xl p-6 sm:p-8 flex flex-col relative overflow-hidden group min-w-[calc(100vw-2.5rem)] sm:min-w-[22rem] md:min-w-[28rem]"
    >
      <span
        className={`font-mono text-3xl sm:text-4xl text-black/10 font-semibold absolute top-6 sm:top-8 right-6 sm:right-8 transition-colors z-10 ${step.hoverColor}`}
      >
        {step.number}
      </span>
...
export default function Process() {
  ...
  return (
    <section id="process" className="horizontal-scroll-wrapper bg-white">
      <div className="horizontal-scroll-sticky">
        <div className="relative md:absolute top-0 md:top-12 z-10 left-0 right-0 px-5 sm:px-8 lg:px-12 mb-6 md:mb-0">
